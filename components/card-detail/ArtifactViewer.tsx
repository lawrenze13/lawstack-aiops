...
export function ArtifactViewer({ kind, filename, markdown, isStale }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[color:var(--border)] px-4 py-2">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={copy}
            className="rounded border border-[color:var(--border)] px-2 py-0.5 text-[11px] hover:bg-[color:var(--surface-secondary)]"
          >
      ...
      <article className="flex-1 overflow-y-auto bg-[color:var(--background)]">
        <div className="mx-auto max-w-3xl px-6 py-6">
          ...
          {kind === "review" && verdict === null ? (
            <div className="mb-4 rounded-md border border-[color:var(--border)] bg-[color:var(--surface-secondary)]/40 px-3 py-2 text-xs text-[color:var(--muted)]">
          ...
          <div className="prose prose-sm max-w-none text-[color:var(--foreground)] prose-headings:text-[color:var(--foreground)] prose-strong:text-[color:var(--foreground)] prose-pre:rounded-md prose-pre:bg-[color:var(--surface-secondary)] prose-pre:p-3 prose-pre:text-xs prose-code:text-[12px] prose-code:before:content-none prose-code:after:content-none prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-li:my-0.5 prose-h1:mt-4 prose-h1:mb-2 prose-h2:mt-4 prose-h2:mb-2 prose-h3:mt-3 prose-h3:mb-1.5 prose-blockquote:border-l-4 prose-blockquote:border-[color:var(--border)] prose-blockquote:pl-3 prose-blockquote:italic prose-table:text-xs">
