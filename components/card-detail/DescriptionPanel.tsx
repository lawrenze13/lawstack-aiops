...
export function DescriptionPanel({ descriptionMd, jiraKey, jiraUrl }: Props) {
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded-lg border border-[color:var(--color-border)]">
      <div className="flex items-center justify-between border-b border-[color:var(--color-border)] px-3 py-2">
        ...
        {jiraUrl ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="font-mono text-[10px] text-[color:var(--color-muted-foreground)] hover:underline"
            title="Open in Jira"
          >
            {jiraKey} ↗
          </a>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
        {descriptionMd.trim() ? (
          <div className="prose prose-sm max-w-none whitespace-pre-wrap text-xs leading-relaxed text-[color:var(--color-foreground)] prose-pre:rounded prose-pre:bg-[color:var(--color-muted)] prose-pre:p-2 prose-pre:text-[11px] prose-code:text-[11px] prose-code:before:content-none prose-code:after:content-none prose-p:my-1.5 prose-headings:mt-2 prose-headings:mb-1">
