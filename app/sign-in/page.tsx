    ...
    <main className="mx-auto flex min-h-screen max-w-sm flex-col justify-center gap-6 p-8">
      <header>
        ...
        <p className="text-sm text-[color:var(--color-muted-foreground)]">
          Sign in with your <span className="font-mono">@{env.ALLOWED_EMAIL_DOMAIN}</span> Google
          account.
        </p>
      </header>
      ...
      {error ? (
        <div className="rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-700">
          Sign-in was rejected. Make sure you used your @{env.ALLOWED_EMAIL_DOMAIN} account.
        </div>
      ) : null}
